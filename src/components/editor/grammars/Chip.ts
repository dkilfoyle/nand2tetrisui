...
import { bin } from "@nand2tetris/web-ide/simulator/src/util/twos.js";
...
export class Bus implements Pin {
  ...
  ensureWidth(newWidth: number) {
    ...
    if (this.width < newWidth) {
      ...
      this.state = [...this.state, ...range(this.width, newWidth).map(() => LOW as Voltage)];
    }
  }
...
export class OutSubBus extends Bus {
  ...
  override set busVoltage(voltage: number) {
    this.bus.busVoltage = (voltage & mask(this.width + this.start)) >> this.start;
  }
...
export function parsePinDecl(toPin: string): {
  ...
  const { pin, w } = toPin.match(/(?<pin>[a-zA-Z]+)(\[(?<w>\d+)\])?/)?.groups as {
    pin: string;
    w?: string;
  };
  return {
...
export function parseToPin(toPin: string): {
  ...
  const { pin, i, j } = toPin.match(/(?<pin>[a-z]+)(\[(?<i>\d+)(\.\.(?<j>\d+))?\])?/)?.groups as { pin: string; i?: string; j?: string };
  return {
    pin,
...
export class Chip {
  ...
  constructor(
    ...
    for (const out of outs) {
      const { pin, width = 1 } =
        (out as { pin: string }).pin !== undefined ? (out as { pin: string; width: number }) : parsePinDecl(out as string);
      this.outs.insert(new Bus(pin, width));
    }

    for (const internal of internals) {
      const { pin, width = 1 } =
        (internal as { pin: string }).pin !== undefined ? (internal as { pin: string; width: number }) : parsePinDecl(internal as string);
  ...
  private wireInPin(part: Chip, to: PinSide, from: PinSide) {
    ...
    // partPin = a = to.name
    // chipPin = orout = from.name
    // chipPin.next = partPin
    // orout.next = a
    // Need to create an edge from ports[orout] to And[id].a
...
function setBus(busses: Pinout, pin: Pin) {
  busses[pin.name] = bin((pin.busVoltage & mask(pin.width)) << (pin as unknown as { start: number }).start ?? 0);
  return busses;
}
